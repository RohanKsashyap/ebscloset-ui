  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        ...
        setCatalog((all || []).filter((p) => (p.type || '').toLowerCase() === 'party' || (p.occasion || '').toLowerCase().includes('party')));
      } catch {
        setError('Failed to load party dresses');
        setCatalog([]);
      } finally {
        setLoading(false);
      }
    ...
    <main className="bg-white scroll-gradient-blur page-enter">
      <section className="py-24 px-6 lg:px-12 max-w-screen-2xl mx-auto">
        <div className="text-center mb-16">
          <h1 className="font-headline text-4xl md:text-6xl lg:text-7xl mb-6 text-hot-pink">Party Dresses</h1>
