  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        ...
        setCatalog((all || []).filter((p) => (p.category || '').toLowerCase().startsWith('age')));
      } catch {
        setError('Failed to load age group dresses');
        setCatalog([]);
      } finally {
        setLoading(false);
      }
    ...
    <main className="bg-white scroll-gradient-blur page-enter">
      <section className="py-24 px-6 lg:px-12 max-w-screen-2xl mx-auto">
        <div className="text-center mb-16">
          <h1 className="font-serif text-4xl md:text-6xl lg:text-7xl mb-6 text-hot-pink">By Age</h1>
